...
describe('TestWrapper', () => {
    ...
    it('creates a new TestLog instance with name matching expectation', async () => {
        let tw: TestWrapper = await new TestWrapper().init(() => expect(true).toBeTruthy());
    ...
    it('appends \'because\' reason to logger name if specified in options', async () => {
        let tw: TestWrapper = await new TestWrapper().init(() => expect(true).toBeTruthy(), {because: 'true is always true'});
    ...
    describe('because method', () => {
        it('will log success result for passed in testIds on successful completion', async () => {
            ...
            let tw: TestWrapper = await new TestWrapper().init(() => expect(false).toBeFalsy(), options);
        ...
        it('will log failed result for passed in testIds on a failed completion', async () => {
            let options: ITestWrapperOptions = {testCases: ['C1234', 'C2345']};
            let tw: TestWrapper = await new TestWrapper().init(() => false, options);
        ...
        it('will skip execution if all tests should not be run', async () => {
            ...
            let tw: TestWrapper = await new TestWrapper().init(() => notExpected = true, options);
